...
function FormSubmitComponent({
  ...
  const submitForm = async () => {
    ...
    if (!validForm) {
      ...
      toast({
        title: "Error",
        description: "pleace check the form for errors",
        variant: "destructive",
      });
      return;
    }
      ...
      <div className="flex justify-center w-full h-full items-center p-8">
        <div className="max-w-[620px] flex flex-col gap-4 flex-grow bg-background p-8 overflow-y-auto border shadow-xl shadow-blue-700 rounded">
          ...
          <p className="text-muted-foreground">
            Thanks you for subbmitting the form.You can close this page now.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="flex justify-center w-full h-full items-center p-8">
      <div
        key={renderKey}
        className="max-w-[620px] flex flex-col gap-4 flex-grow bg-background p-8 overflow-y-auto border shadow-xl shadow-blue-700 rounded">
        {content.map((element) => {
